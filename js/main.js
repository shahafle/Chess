...
function getAllPossibleCoordsRook(pieceCoord) {
    ...
    // var isEmpty = true;
    // var diff = 1;
    // //checks moves in column
    // for (var i = 0; i < 2; i++) {
    //     var nextCoord = { i: pieceCoord.i, j: pieceCoord.j }
    //     isEmpty = true;
    //     while (isEmpty) {
    //         nextCoord = { i: nextCoord.i + diff, j: pieceCoord.j }
    //         if (nextCoord.i < 8 && nextCoord.i > -1 && isEmptyCell(nextCoord)) {
    //             res.push(nextCoord);
    //         } else {
    //             isEmpty = false;
    //         }
    ...
    //     isEmpty = true;
    //     while (isEmpty) {
    //         nextCoord = { i: pieceCoord.i, j: nextCoord.j + diff }
    //         if (nextCoord.j < 8 && nextCoord.j > -1 && isEmptyCell(nextCoord)) {
    //             res.push(nextCoord);
    //         } else {
    //             isEmpty = false;
    //         }
    ...
    for (var k = 0; k < 2; k++) {
        for (var i = 0; i < 2; i++) {
            for (var j = 0; j < 2; j++) {
                ...
                isEmpty = true;
                while (isEmpty) {
                    nextCoord = { i: nextCoord.i + diffI, j: nextCoord.j + diffJ }
                    if (nextCoord.i < 8 && nextCoord.i > -1 & nextCoord.j < 8 && nextCoord.j > -1 && isEmptyCell(nextCoord)) {
                        res.push(nextCoord);
                    } else {
                        isEmpty = false;
                    }
...
function getAllPossibleCoordsQueen(pieceCoord) {
    var tempExtractCoords1 = getAllPossibleCoordsRook(pieceCoord);
    // for (var i = 0; i < tempExtractCoords.length; i++) {
    //     res.push(tempExtractCoords[i]);
    // }
    var tempExtractCoords2 = getAllPossibleCoordsBishop(pieceCoord);
    // for (var i = 0; i < tempExtractCoords.length; i++) {
    //     res.push(tempExtractCoords[i]);
    // }
    var res = tempExtractCoords1.concat(tempExtractCoords2);
    return res;
}